...
export function MonthlyClientPerformance({
  ...
  const filteredTotals = useMemo(() => {
    const totalFacturas = filteredItems.reduce((total, item) => total + item.total, 0);
    const totalConf = filteredItems.reduce((total, item) => total + item.aud_conf, 0);
    const totalRech = filteredItems.reduce((total, item) => total + item.aud_rech, 0);
    const totalDocs = filteredItems.reduce((total, item) => total + item.total_doc, 0);
  ...
  return (
    <SectionCard
      ...
      actions={
        <div className="flex items-center gap-2" aria-label="Seleccionar año">
    ...
    >
      {error ? (
        <Alert variant="destructive" className="mb-5" aria-live="polite">
          ...
          <AlertDescription className="flex items-center justify-between gap-3">
            <span>{error}</span>
            <Button type="button" variant="ghost" size="sm" onClick={() => handleYearChange(selectedYear)}>
              Reintentar
            </Button>
          </AlertDescription>
        </Alert>
      ...
      <div className="grid overflow-hidden rounded-md border border-border sm:grid-cols-2 xl:grid-cols-4">
        <SummaryMetric
          ...
          detail={`Auditadas en ${selectedYear}`}
        />
        <SummaryMetric
          icon={<FileText />}
          label="Soportes IA"
