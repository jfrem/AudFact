...
export function InvoicesTable({
  ...
  const rows = invoices.map((item, index) => {
    ...
    const disId = String(item.dis_id ?? item.DisId ?? "N/D");

    return { dispensa, nitSec, disId, index };
  });
  ...
  return (
    <>
      <ConfirmDialog
        ...
        description={`Se ejecutará la auditoría IA sobre la entrega ${auditTarget?.disId ?? ""}. El proceso puede tomar entre 10 y 60 segundos.`}
        confirmLabel="Auditar"
        loading={navigation.isPending && pendingRoute === `audit:${auditTarget?.disId ?? ""}`}
        onConfirm={() => {
          if (auditTarget) {
      ...
      {invoices.length > 0 ? (
        <Table>
          ...
          <TableBody>
            {rows.map(({ dispensa, nitSec, disId, index }) => {
              return (
                <TableRow
                  key={`${disId}-${index}`}
                ...
                >
                  ...
                  <TableCell className="font-mono text-xs text-slate-400" title={`#${disId}`}>
                    #{disId}
                  </TableCell>
