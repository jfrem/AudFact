...
### feat
- **Canal VIP de Auditorías 1:1 con Streams Prioritarios (Prioridad $P_0$ de Ventanilla)**: Se implementó una arquitectura multi-stream dual nativa en Redis 7 (`*.priority` y `*.batch`) que garantiza evaluación inmediata para auditorías individuales interactivas (`POST /audit/single`) sin que queden bloqueadas detrás de colas de lotes masivos.
  - El pool compartido de workers de pipeline (`DocumentAuditOrchestrator`, `AttachmentDownloadWorker`, `DocumentExtractionWorker`, `DocumentNormalizer`, `RulesEvaluationWorker`, `AuditPersistenceWorker`) consume concurrentemente mediante `XREADGROUP ... STREAMS key.priority key.batch > >`, evaluando posicionalmente de izquierda a derecha.
  - Se eliminaron por completo aliases ambiguos y código muerto bajo **Clean Rebuild Policy**, consolidando `abstract protected function streams(): array` en el consumidor base y resolviendo ACKs y métricas desagregadas por stream de origen exacto.
  - Archivos creados/modificados: `core/RedisClient.php`, `app/Services/Audit/Pipeline/AuditEventPublisher.php`, `app/Services/Audit/Pipeline/AuditEventConsumer.php`, `app/Services/Audit/Pipeline/DocumentAuditOrchestrator.php`, `app/Services/Audit/Pipeline/AttachmentDownloadWorker.php`, `app/Services/Audit/Pipeline/DocumentExtractionWorker.php`, `app/Services/Audit/Pipeline/DocumentNormalizer.php`, `app/Services/Audit/Pipeline/RulesEvaluationWorker.php`, `app/Services/Audit/Pipeline/AuditPersistenceWorker.php`, `app/Services/Audit/Pipeline/AuditPersistenceQueue.php`, `app/Services/Audit/Pipeline/BatchRequestedWorker.php`, `app/Controllers/AuditController.php`, `app/Controllers/ObservabilityController.php`, `.agent/skills/audfact-audit-gemini/SKILL.md`, `plans/sdd-interactive-vip-priority-stream.md`, y suites completas de pruebas unitarias.
  - Impacto: Tiempos de espera en cola para ventanilla / sala de dispensación reducidos de minutos a $\le 3\text{ ms}$ en concurrencia con lotes masivos.

- **Módulo de Rendimiento y Producción Mensual por EPS en Dashboard (`/dashboard`)**: Se incorporó el endpoint `GET /audit/stats/monthly` y el componente reactivo `MonthlyClientPerformance` en Next.js bajo los estándares de **/impeccable**. Permite visualizar y segmentar el rendimiento histórico por año (2026, 2025, 2024), con KPIs de impacto (Total Facturas, Soportes Evaluados por IA, % Conformidad Global, Rechazos), tabla detallada con filtro por texto, micro-barras de progreso integradas por fila y pie de tabla (`<tfoot>`) con totales consolidados calculados automáticamente.
  - Archivos creados/modificados: `app/Models/AuditStatusModel.php`, `app/Controllers/AuditController.php`, `app/Routes/web.php`, `frontend/lib/schemas/domain.ts`, `frontend/lib/api/endpoints.ts`, `frontend/lib/api/audfact.ts`, `frontend/components/dashboard/monthly-client-performance.tsx`, `frontend/app/(dashboard)/dashboard/page.tsx`, `tests/Controllers/AuditControllerTest.php`, `plans/sdd-dashboard-depuration-and-monthly-performance.md`.
  - Impacto: Transformación del Dashboard en un Centro de Control Ejecutivo y Operativo con visibilidad directa del volumen real y tasas de éxito de la IA por cliente.

### refactor
- **Depuración Integral del Dashboard y Erradicación de Redundancias (Clean Rebuild)**: Se depuró la vista `/dashboard` eliminando widgets y secciones obsoletas que duplicaban información ya disponible en pantallas dedicadas. Se eliminaron físicamente `frontend/components/dashboard/priority-audit-table.tsx` (redundante con `/audit/results`), `frontend/components/dashboard/stat-card.tsx`, `frontend/components/dashboard/async-queue-summary.tsx` y `frontend/components/dashboard/state-distribution-chart.tsx`, así como las barras horizontales intermedias, simplificando la carga de datos y reduciendo el ruido cognitivo a cero.
  - Impacto: Carga inicial de la aplicación sustancialmente más rápida y jerarquía visual impecable enfocada en Hero KPIs y Analítica de Negocio.
