...
| `AuditConfigModel` | `Discolnet.dbo.AudDisp` + `Discolnet.dbo.AudDispCampo` | Configuración dinámica por cliente; lee y reemplaza campos activos, severidad, descripción visual, `TipoDato`, `CodigoCampo`, aplicabilidad por servicio `AplicaServicio` y alcance multi-ítem `EsMultiItem` |
