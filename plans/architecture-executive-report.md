...
*   **Trade-Offs**:
    ...
    *   *Desventaja*: Redis almacena la cola en memoria RAM por defecto. Para mitigar el riesgo de pérdida de eventos en caso de reinicio forzado del servidor Redis, se configuró la persistencia mediante **RDB snapshots** (`--save 60 1000`: un snapshot cada 60 segundos si hubo al menos 1000 escrituras). Nota: AOF no está habilitado; existe una ventana teórica de pérdida de hasta 60 segundos ante un crash abrupto, mitigada por la velocidad de encolamiento y la idempotencia del pipeline.
