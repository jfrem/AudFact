...
final class AuditDlqControllerTest extends TestCase
{
    ...
    public function testJobReopenCompensationFailureRecordsReconciliation(): void
    {
        $auditId = AuditEvent::uuidV4();
        $jobId = AuditEvent::uuidV4();
        $original = AuditEvent::create(
        ...
        $stateStore->expects($this->once())
            ->method('reopenAuditForReprocess')
            ->willReturn(true);
        // revertReprocess fails all 3 attempts
        $stateStore->expects($this->exactly(3))
            ->method('revertReprocess')
            ->willReturn(false);
        // Should record failed reconciliation
        $stateStore->expects($this->once())
            ->method('recordFailedReconciliation')
            ->with($this->equalTo($auditId), $this->callback(function (array $data) use ($jobId): bool {
                return ($data['state_reverted'] ?? null) === false
                    && ($data['job_id'] ?? null) === $jobId
                    && str_contains((string) ($data['error'] ?? ''), 'fallo de coordinación');
            }))
            ->willReturn(true);
        ...
        $response = self::captureResponse(static fn() => $controller->reprocess());
        $this->assertSame(409, $response->getCode());
        $this->assertStringContainsString('No se pudo reabrir la auditoría en el job batch', $response->getData()['message'] ?? '');
    }

    public function testIndexIncludesReconciliationAnomalies(): void
    {
        $redis = $this->createMock(RedisClient::class);
        $redis->expects($this->once())
            ->method('xRange')
        ...
        $redis->expects($this->once())
            ->method('hGet')
            ->with('telemetry:async_metrics', 'reconciliation_anomalies')
            ->willReturn('3');
        ...
        $this->assertSame(3, $data['reconciliation_anomalies']);
    }
