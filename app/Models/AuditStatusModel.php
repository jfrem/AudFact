...
class AuditStatusModel extends Model
{
    ...
    /**
     * Obtiene la configuración de auditoría para un cliente específico.
     * Consulta la tabla AudDispCampo para obtener campos habilitados y overrides.
     */
    public function getConfigByClient(string $clientId): ?array
    {
        $sql = "SELECT
                    FacNitSec,
                    NitMedDocId,
                    ...
                    Rol,
                    OmitirSi
                FROM Discolnet.dbo.AudDispCampo WITH (NOLOCK)
                WHERE FacNitSec = :clientId";
        ...
        $docTypes = $attModel->getDocumentTypes();
        $docMap = [];
        foreach ($docTypes as $dt) {
            $docMap[$dt['DocumentoId']] = $dt['DocumentoNombre'];
        }

        $docs = [];
        foreach ($rows as $row) {
            $docId = $row['NitMedDocId'];
            $docName = $docMap[$docId] ?? "DOC_{$docId}";

            if (!isset($docs[$docName])) {
                $docs[$docName] = ['fields' => []];
            }

            $docs[$docName]['fields'][$row['CampoNombre']] = [
                'tipoCampo' => $row['TipoCampo'],
                'enabled' => (bool)$row['Activo'],
                'descripcionOverride' => $row['DescripcionOverride'],
                'severityOverride' => $row['SeveridadOverride'],
                'orden' => $row['Orden'],
    ...
    /**
     * Guarda la configuración de auditoría para un cliente.
     * Usa MERGE para actualizar o insertar en AudDispCampo.
     */
    public function saveConfigByClient(string $clientId, array $data): bool
    {
        ...
        try {
            ...
            $sql = "MERGE Discolnet.dbo.AudDispCampo AS target
                    USING (SELECT :FacNitSec AS FacNitSec, :NitMedDocId AS NitMedDocId, :CampoNombre AS CampoNombre) AS source
                    ON target.FacNitSec = source.FacNitSec
                       AND target.NitMedDocId = source.NitMedDocId
                       AND target.CampoNombre = source.CampoNombre
                    WHEN MATCHED THEN
                        UPDATE SET
                            target.TipoCampo = :TipoCampo,
                            target.Activo = :Activo,
                            target.DescripcionOverride = :DescripcionOverride,
                            target.SeveridadOverride = :SeveridadOverride,
                            target.Orden = :Orden,
                            target.Rol = :Rol,
                            target.OmitirSi = :OmitirSi
                    WHEN NOT MATCHED THEN
                        INSERT (FacNitSec, NitMedDocId, CampoNombre, TipoCampo, Activo, DescripcionOverride, SeveridadOverride, Orden, Rol, OmitirSi)
                        VALUES (source.FacNitSec, source.NitMedDocId, source.CampoNombre, :TipoCampo, :Activo, :DescripcionOverride, :SeveridadOverride, :Orden, :Rol, :OmitirSi);";

            $stmt = $writeDb->prepare($sql);

            foreach ($data['documents'] as $docName => $docConfig) {
                $docId = $docConfig['docId'] ?? null;
                if (!$docId || !isset($docConfig['fields'])) continue;

                foreach ($docConfig['fields'] as $field) {
                    $omitirSi = $this->normalizeOmitirSi($field['omitirSi'] ?? null);
                    $stmt->execute([
                        ':FacNitSec' => $clientId,
                        ':NitMedDocId' => $docId,
                        ':CampoNombre' => $field['campoNombre'],
                        ':TipoCampo' => $field['tipoCampo'] ?? 'E',
                        ':Activo' => ($field['enabled'] ?? true) ? 1 : 0,
                        ':DescripcionOverride' => $field['descripcionOverride'] ?? null,
                        ':SeveridadOverride' => $field['severityOverride'] ?? 'ALTA',
                        ':Orden' => $field['orden'] ?? 0,
                        ':Rol' => $field['rol'] ?? null,
                        ':OmitirSi' => $omitirSi,
                    ]);
            ...
            Logger::error("Error salvando configuración de cliente {$clientId}: " . $e->getMessage());
            return false;
        }
    }

    /**
     * Normaliza el valor de OmitirSi a JSON canónico antes de persistir.
     * Acepta array (lo serializa), string JSON limpio (lo retorna tal cual),
     * string double-encoded (lo normaliza) o null/vacío (retorna null).
     */
    private function normalizeOmitirSi(mixed $value): ?string
    {
        ...
        if (is_string($value)) {
            ...
            // Intentar corregir double-encoding
